...
ShinePet 包含4种不同的动画状态：

| 动画 | 触发方式 | 描述 |
|------|---------|------|
| 🧘 **静止 (Idle)** | 默认状态 | 轻微上下浮动，表示静静陪伴 |
| 👋 **挥手 (Wave)** | 单击宠物 | 多帧动画：右手→左手→双手，完整打招呼一次 |
| 🚶 **走路 (Walk)** | 双击宠物 | 随机方向移动，3秒内完成移动→回归循环 |
...
- 👆 **单击**：在静止和挥手状态间切换（静止 ↔ 挥手）
- 👆👆 **双击**：触发走路动画，随机方向移动后自动回归原位，持续3秒
- 🖱️ **右键**：打开设置菜单（可打开选项页面配置天气等功能）
...
|------|---------|------|
| 🧘 **静止 (Idle)** | 默认状态 | 轻微上下浮动，表示静静陪伴 |
| 👋 **挥手 (Wave)** | 单击宠物 | 友好的打招呼动作 |
| � **走路 (Walk)** | 连续单击 | 悠闲漫步，持续2秒 |
| 🏃 **跑步 (Run)** | 连续单击 | 快速移动，持续1.5秒 |
| 💃 **跳舞 (Dance)** | 双击宠物 | 欢快的摇摆，持续3秒 |
